compare.narr.wrf <- function(varname, vardim, subset, path, pattern, res = c("3-hourly","6-hourly","daily","monthly"), calc = c("ASIS","MEAN","RMSE"), pct = FALSE, narrpath, narrfile = NULL, prlevs = c(500, 700, 850)){
    ...
    WRFVAR = c("hgt","U","V","U10","V10","PSFC","TT","T2","RH2","SH","SH2") # "PR","RH","SST"
    NARFILE = c("hgt.yyyymm.nc","uwnd.yyyymm.nc","vwnd.yyyymm.nc","uwnd.10m.yyyy.nc","vwnd.10m.yyyy.nc","pres.sfc.yyyy.nc","air.yyyymm.nc","air.2m.yyyy.nc","rhum.2m.yyyy.nc","shum.yyyymm.nc","shum.2m.yyyy.nc","apcp.yyyy.nc") # narr file name pattern
    NARVAR = c("hgt","uwnd","vwnd","uwnd","vwnd","pres","air","air","rhum","shum","shum","apcp") # narr variable lat(y,x), lon(y,x), var(time, (level,) y, x), level [hPa], apcp [kg/m^2]
    ...
    narrvar = get.narr.tvar(narrvarname, subset, sublat = wrflatrng, sublon = wrflonrng, narrpath, narrfile, calc = "ASIS", prlevs = prlevs, return_tstamp = TRUE)
    # convert NARR variable to specified resolution
    narrvar = condense(narrvar$var, narrvar$timestamp, calc = calcC, toscale = res)
