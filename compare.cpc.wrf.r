compare.cpc.wrf <- function(path, pattern, subset, calc = c("ASIS","MEAN","RMSE"), toscale = c("monthly","annual"), pct = FALSE, cpcpath, cpcfile = "precip.V1.0.yyyy.nc"){
    ...
    # CPC path
    if (is.null(cpcpath)){
        cpcpath = "/nfs/gpfs/PAS0661/DATA/CPC/"
    }

    # read the WRF precipitation (need to convert to daily)
    
    vvv = get.wrf.tvar(path, pattern, subset, "RAINNC", calc = "ASIS", prlevs = prlevs, offset = offset, return_tstamp = TRUE)
    # convert the WRF variable to monthly or annual
    wrfvar = condense(vvv$var, vvv$timestamp, calc = "SUM", toscale = toscale)
    ...
    if (length(cpcrain) == 2){
        cpcrain_intrp = cpcrain[ cpclon > min(wrflon, na.rm=TRUE) & cpclon < max(wrflon, na.rm=TRUE), cpclat > min(wrflat, na.rm=TRUE) & cpclat < max(wrflat, na.rm=TRUE) ]
    } else {
        cpcrain_intrp = cpcrain[ cpclon > min(wrflon, na.rm=TRUE) & cpclon < max(wrflon, na.rm=TRUE), cpclat > min(wrflat, na.rm=TRUE) & cpclat < max(wrflat, na.rm=TRUE),  ]
    }
    
    diff = wrfvar_intrp - cpcrain_intrp
    if (pct){
        diff = diff / cpcrain_intrp * 100
    }

    if (calc == "ASIS"){
        return( diff )
    } else if (calc == "MEAN"){
        return( apply(diff, MARGIN = c(1,2), FUN = mean, na.rm = FALSE) )
    } else if (calc == "RMSE"){
        return( apply(diff, MARGIN = c(1,2), FUN = sd, na.rm = FALSE) )
    }
}
